...
import { ProductionCalendarView, PRODUCTION_CALENDAR } from "./components/prodplan";
    ...
    <main className={role==="floor"?"floor-shell":"app-shell"}>
      ...
      <div className={role==="floor"?"floor-app":"app-layout"}>
        ...
        <section className={role==="floor"?"floor-workspace":"workspace"}>
          {!loaded?<p className="intro">Loading company records…</p>:currentNav===ORDER_FLOW?<OrderFlowView/>:currentNav===PRODUCTION_CALENDAR?<ProductionCalendarView/>:currentNav==="My account"?<SalesView nav="My account"/>:role==="sales"?<SalesView nav={currentNav}/>:role==="owner"?<OwnerView nav={currentNav}/>:<FloorView nav={currentNav} line={currentLine} setLine={setLine}/>}
        </section>
      </div>
    </main>
