...
export function OrderModal({kind,close,presetCustomer,fromQuote}:{kind:"order"|"quote"|"invoice";close:()=>void;presetCustomer?:string;fromQuote?:DocumentRecord}){
  ...
  const [ship,setShip]=useState(ships[0]?.id||"pickup");
  ...
  const draft:OrderRecord={id:"draft",customerId:custId,item:"",cases:0,quantity:priced.reduce((a,l)=>a+l.quantity,0),due,status:"",payment:"",lines:priced,shipMethod:ship,discount:disc};
  ...
  const submit=(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();if(!cust||!priced.length)return;
    if(kind==="order"){
      ...
      const doc:DocumentRecord={id,kind,customerId:cust.id,item:priced.map(l=>l.item).join(" + "),cases:t.cases,quantity:draft.quantity,rate:priced[0].rate,discount:disc,shipping:t.ship,status:kind==="quote"?(needsApproval?"Awaiting approval":"Sent"):"Open",due:dueLabel,paid:0,note:custNote,qbSynced:data.settings.quickBooks.connected};
      commit(v=>({...v,documents:[doc,...v.documents],customers:v.customers.map(c=>c.id===cust.id?{...c,stage:c.kind==="lead"&&kind==="quote"?"Quote sent":c.stage,balance:kind==="invoice"?c.balance+t.total:c.balance}:c),
      ...
      notify(kind==="quote"?(needsApproval?`Quote ${id} sent to the owner for approval`:`Quote ${id} saved and ready to email`):`Invoice ${id} created${data.settings.quickBooks.connected?" · synced to QuickBooks":""}`,kind==="quote"?"Quotes":"Invoices");close();
    }
  };
  ...
  return <Shell wide title={title} eyebrow={kind==="order"?"Sale to shipment":"Sales document"} onSubmit={submit} close={close} submitLabel={kind==="order"?(needsApproval?"Save & send for approval":"Place order"):kind==="quote"?"Save quote":"Create invoice"}>
    ...
    <div className="order-lines">{priced.map((l,i)=>{const r=finished.find(x=>x.item===l.item);const row=data.inventory.find(x=>x.item===l.item);const free=row?freeStock(row):0;return <div className="form-grid order-line" key={i}>
    ...
    <div className="form-grid">
      <label>Shipping<select value={ship} onChange={e=>setShip(e.target.value)}>{ships.map(s=><option key={s.id} value={s.id}>{s.name} — {s.perCase?`${usd2(s.perCase)}/box`:s.rate?usd2(s.rate):"free"}</option>)}</select></label>
