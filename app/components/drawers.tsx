...
export function RecordDrawer({id,close}:{id:string;close:()=>void}){
  ...
  if(order){
    ...
    const setStage=(n:number,extra?:(v:typeof data)=>Partial<typeof data>)=>act(v=>({...v,orders:v.orders.map(x=>x.id===order.id?{...x,stage:n,status:STAGES[n]}:x),activities:[activity(order.customerId,"Order status updated",`${order.id} · ${STAGES[n]}`),...v.activities],...(extra?extra(v):{})}),"order.status",`${order.id} → ${STAGES[n]}`,`Order ${order.id} is now ${STAGES[n]}`);
    const ship=()=>setStage(4,v=>({inventory:v.inventory.map(row=>{const l=t.lines.find(l=>l.item===row.item);return l?{...row,onHand:Math.max(0,row.onHand-l.quantity),committed:Math.max(0,row.committed-l.quantity)}:row}),notices:[{id:uid("n"),title:`${order.id} shipped`,detail:`${customer?.name} · ready to invoice`,urgent:false,read:false,createdAt:now(),target:"Invoices"},...v.notices]}));
    ...
    const makeInvoice=async(paidNow=false)=>{const invId=nextId("INV-",data.documents.filter(x=>x.kind==="invoice").map(x=>x.id),1042);const due=new Date();due.setDate(due.getDate()+30);
      ...
      act(v=>({...v,documents:[{id:invId,kind:"invoice",customerId:order.customerId,orderId:order.id,item:t.lines.map(l=>l.item).join(" + "),cases:t.cases,quantity:order.quantity,rate:t.lines[0]?.rate||0,discount:order.discount||0,shipping:t.ship,status:paidNow?"Paid":"Open",due:paidNow?"—":fmtDay(due),paid:paidNow?t.total:0,qbSynced:!!qbo.qboId,qboId:qbo.qboId,qboDocNumber:qbo.docNumber,note:order.invoiceNote},...v.documents],
        orders:v.orders.map(x=>x.id===order.id?{...x,invoiceId:invId,stage:paidNow?6:5,status:paidNow?"Paid":"Invoiced",payment:paidNow?"Paid":`${usd2(t.total)} due`}:x),
    ...
    const pay=async()=>{let pid="";if(inv?.qboId&&customer?.qboId){try{const r=await qboCall({op:"payment.create",qboId:inv.qboId,customerQboId:customer.qboId,amount:t.total,method:window.prompt("How was it paid? (check, ACH, card, cash)","check")||"payment"});pid=String(r.paymentId||"")}catch(e){notify(`QuickBooks: ${e instanceof Error?e.message:"payment failed"}`,"Invoices",true);return}}
      act(v=>({...v,documents:v.documents.map(x=>x.id===order.invoiceId?{...x,status:"Paid",paid:t.total,paymentQboId:pid||x.paymentQboId}:x),orders:v.orders.map(x=>x.id===order.id?{...x,stage:6,status:"Paid",payment:"Paid"}:x),customers:v.customers.map(c=>c.id===order.customerId?{...c,balance:Math.max(0,c.balance-t.total)}:c),activities:[activity(order.customerId,"Payment received",`${order.invoiceId} · ${usd2(t.total)}${pid?" · QuickBooks payment":""}`),...v.activities]}),"invoice.payment",`${order.invoiceId} paid`,`Payment received — ${order.invoiceId} · ${usd2(t.total)}${pid?" · recorded in QuickBooks":""}`,"Invoices",true)};
    ...
    body=<>
      ...
      <DetailField label="Needed" value={`${order.due}${order.rep?` · taken by ${order.rep}`:""}`}/>
      ...
      <DetailField label="Invoice" value={order.invoiceId?`${order.invoiceId} · ${st===6?"Paid":inv?.status||"Open"}${inv?.qboId?` · QuickBooks #${inv.qboDocNumber||inv.qboId}`:""}`:"Not yet invoiced"}/>
    </>;
    footer=<>
      ...
      {!needsApproval&&st===0&&owner&&!wos.length&&<button className="secondary" onClick={()=>{close();setModal("workorder",order.id)}}>+ Work order</button>}
      {!needsApproval&&st<3&&owner&&<button className="primary" onClick={()=>setStage(st+1)}>Mark {STAGES[st+1]}</button>}
      {st===3&&<button className="primary" onClick={ship}>Mark shipped</button>}
      {(st===4||(st===3&&order.shipMethod==="pickup"))&&!order.invoiceId&&<button className="primary" onClick={()=>makeInvoice(false)}>Create invoice{data.settings.quickBooks.connected?" in QuickBooks":""}</button>}
      {st>=3&&!order.invoiceId&&customer?.terms.includes("Card")&&<button className="secondary" onClick={()=>makeInvoice(true)}>Take card payment</button>}
      {order.invoiceId&&st<6&&<><button className="secondary" onClick={sendLink}>Email invoice / pay link</button><button className="primary" onClick={pay}>Record payment</button></>}
    </>;
  }
  // ---------------- DOCUMENT ----------------
  else if(doc){
